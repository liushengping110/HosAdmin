...
public class Fragment_Components extends Fragment implements PopDissListener,DeviceMonitiorAdapter.MonViewItemOnClick{
    //控件是否已经初始化
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if(view==null){
            view=inflater.inflate(R.layout.fragment_components,null);
            isCreateView = true;
            initUI();
            initTypePop();
            setListener();
    ...
    public void setData(){
        for(int i=0;i<10;i++){
            ...
            switch (i){
                ...
                default:
                    monitiorDetail.setPartName("银行卡读卡器");
                    monitiorDetail.setPartStatusContent("加载驱动失败");
                    break;
            }
